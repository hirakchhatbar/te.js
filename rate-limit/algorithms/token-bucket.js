...
class TokenBucketRateLimiter extends RateLimiter {
   ...
   * @param {Object} [options.tokenBucket] - Token bucket specific options
   * @param {number} [options.tokenBucket.refillRate] - Tokens added per second
   * @param {number} [options.tokenBucket.burstSize] - Maximum tokens the bucket can hold
   * @throws {Error} If required tokenBucket options are missing
   */
  constructor(options) {
    ...
    if (!this.tokenBucketOptions) {
      throw new Error('TokenBucketRateLimiter requires tokenBucketConfig options');
    }
  }
  ...
  async consume(identifier) {
    ...
    if (stored.tokens < 1) {
      const timeToNextToken = Math.ceil((1 - stored.tokens) / options.refillRate * 1000);
      return {
        success: false,
        remainingRequests: 0,
